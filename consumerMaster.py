...
import psycopg2  # Commented out for disabling TimescaleDB
...
# TimescaleDB Master Setup (Commented out for disabling TimescaleDB)
...
# Function to Write to TimescaleDB Master (Commented Out)
...
try:
    while True:
        ...
        # Convert timestamp to ISO 8601 format (if necessary)
        try:
            iso_timestamp = datetime.fromisoformat(timestamp).isoformat()
        except Exception as e:
            print(f"Invalid timestamp format: {timestamp}. Skipping this record. Error: {e}")
            continue
        ...
        try:
            ...
            print(f"Written to InfluxDB Master: Bed {bed_number}, Heart Rate: {heart_rate}, Timestamp: {iso_timestamp}")
        except Exception as e:
            print(f"Failed to write to InfluxDB Master: {e}")

        # Write to TimescaleDB Master (Commented Out)
    ...
    #timescale_cursor_master.close()  # Commented out
    #timescale_conn_master.close()    # Commented out
